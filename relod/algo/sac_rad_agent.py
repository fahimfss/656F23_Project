...
from relod.algo.replay_buffer import AsyncSMRadReplayBuffer
...
class SACRADLearner(BaseLearner):
    def __init__(self, args, performer=None) -> None:
        ...
        self._obs_queue = self.ctx.Queue()
        self._instructions_queue = self.ctx.Queue()

        if not self._args.async_mode:
            self._replay_buffer = AsyncSMRadReplayBuffer(
                self._args.image_shape, self._args.proprioception_shape, self._args.action_shape,
                self._args.replay_buffer_capacity, self._args.batch_size, self._obs_queue,
        ...
        if self._args.async_mode:
            ...
            self._update_queue = self.ctx.Queue(3)
    ...
    def _update(self, images, propris, actions, rewards, next_images, next_propris, dones):
        ...
        rewards = torch.as_tensor(rewards, device=self._args.device).unsqueeze(-1)
        dones = torch.as_tensor(dones, device=self._args.device).unsqueeze(-1)
    ...
    def _async_update(self):
        ...
        while True:
            try: 
                ...
                st = self._update(batch.images, batch.proprioceptions, batch.actions, batch.rewards, 
                                  batch.next_images, batch.next_proprioceptions, batch.masks)
                self._update_queue.put_nowait(st)     
                
            except queue.Full:
                pass

            if not self._instructions_queue.empty():
                ins = self._instructions_queue.get()
                if ins == 'close':
    ...
    def push_sample(self, ob, action, reward, next_ob, done):
        ...
        self._obs_queue.put((image, propri, action, reward, next_image, next_propri, done))
